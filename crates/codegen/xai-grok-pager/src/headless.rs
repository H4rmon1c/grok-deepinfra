...
fn auth_required_message(interactive: bool) -> String {
    if interactive {
        "No OpenAI Platform API key is configured. Set OPENAI_API_KEY in this shell and retry. \
         A ChatGPT subscription or browser/session token is not an API key."
            .to_string()
    } else {
        "No OpenAI Platform API key is configured. Set OPENAI_API_KEY in the process \
         environment or CI secret store and retry. A ChatGPT session token is not an API key."
            .to_string()
    }
}
