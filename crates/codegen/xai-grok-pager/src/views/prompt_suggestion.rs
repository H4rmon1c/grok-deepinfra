...
pub const PREFERRED_SUGGESTION_MODEL: &str = "gpt-5.6-luna";
...
/// env override > `gpt-5.6-luna` when the catalog offers it > `None`.
///
/// The hint is one tier of the shell-side resolution (env > config.toml >
/// remote settings > this hint > `gpt-5.6-luna` default): the shell
